layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    $("#menuAjax").click(function(){
        ...
        //ajax提交
        $.ajax({
        	url:"/MenuTest/MenuServlet?action=upMenu",
        	type:"post",
        	data:data2,
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("/MenuTest/MenuServlet?action=allButtenType",function (res) {
        		var cs = eval('(' + res + ')');
        		var btntype = $('#btnType');
        ...
        $.get("/MenuTest/MenuServlet?action=menuByType2&type="+type,function (res) {
        	var cs = eval('(' + res + ')');
            var data = cs.data;
            var dom = $("#father");
            if(type == 1){
