...
*/
    ...
    form.on('submit(menuUser)',function(data){
    	var menuname = $("#menuname").val();  //权限
	    var url = $("#url").val();  //请求路径
	    var type = $("#type").val();  //请求类型
	   
	    var button = $("#button").val();  //按钮代码
	    var fatherid = $("#fatherid").val();  //父级
	   /* //权限名
        if(mname == "" || mname == null){
        	layer.msg("权限名不能为空");
        	return false;
		...
		$.post("/Employee/QxianInsert?menuname="+menuname+"&url="+url+"&type="+type+"&button="+button+"&fatherid="+fatherid,function(data){
			var info = eval("("+data+")");
	 		if(info == 1){
    ...
    form.on('select(type)', function(data){
    	var type = data.value;
        var dom = $("#goodsid");
        var mname = $("#mrnuname").val();  //权限
        dom.html("");
        if(type == 3){
        	$("#button").val('');
        	$("#iconclass").css("display","none");
        	$("#btnclss").css("display","block");
        	$.get("/Employee/BtnSelect",function (res) {
        		var cs = eval('(' + res + ')');
        		var btntype = $('#btnType');
        ...
        $.get("/Employee/QxType?type="+type,function (res) {
        	var cs = eval('(' + res + ')');
            var data = cs.data;
            var dom = $("#fatherid");
            if(type == 1){
                ...
                $.each(cs,function (index, item) {
                    html += '<option value="'+item.id+'">'+item.gong+'</option>';
                });
            }
            dom.html(html);
            //重新渲染下拉列表
